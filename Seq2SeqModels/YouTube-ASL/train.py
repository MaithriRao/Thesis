...
def process_model_output(model, features, feature_lengths, labels, one_hot_labels, teacher_force_ratio, padding_index):
    ...
    output = output[:, 1:, :].reshape(-1, output_dim) # [ batch_size * (seq_len -1), num_classes] # [128, 5] # Remove BOS token from output
    # print("Labels shape before reshape:", labels.shape) # [batch_size, num_classes] # [16, 9]
    trg = labels[:, 1:].reshape(-1).long()  # (batch_size * num_classes-1 ) # [128] # Remove BOS token from target labels
...
class EarlyStopping:
  def __init__(self, patience, verbose=False, use_attention=False, delta=0, best_model_path='checkpoint.pth'):
    ...
    self.best_model_path = f'{best_model_prefix}{attention_suffix}.pth'
...
def main(config):
    device = setup_device()
    train_loader, class_weights, val_loader, class_weights_val, _, _, _, _ = setup_dataloaders(config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Youtube-ASL data  training for Sign Language Segmentation')
