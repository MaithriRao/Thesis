...
def train(model, decoder, train_loader, val_loader, loss_fn, loss_fn_val, device, config):
  ...
  for epoch in range(n_epochs):
    ...
    for i, data in tqdm(enumerate(train_loader), total = len(train_loader)):
      ...
      output = model(features, feature_lengths, one_hot_labels, teacher_force_ratio=0.)
    ...
    if frame_f1_score > best_f1:
      ...
      torch.save(model.state_dict(), 'best_model_no_att.pth')
...
class EarlyStopping:
  def __init__(self, patience=10, verbose=False, delta=0, best_model_path='checkpoint_best.pth'):
