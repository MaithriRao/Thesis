...
def train(model, decoder, train_loader, val_loader, loss_fn, loss_fn_val, device, config):
  ...
  for epoch in range(n_epochs):
    ...
    if frame_f1_score > best_f1:
      ...
      torch.save(model.state_dict(), 'best_model_no_att_F1.pth') # best_model_att128_ep100
...
class EarlyStopping:
  def __init__(self, patience=10, verbose=False, delta=0, path='checkpoint_no_att_F1.pth'): # checkpoint_att128_ep100
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description='ASL motion stream')
