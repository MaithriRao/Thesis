...
def beam_search_inference(model, test_loader, device, beam_width):
  ...
  with torch.no_grad():
    for i, data in enumerate(test_loader):
      ...
      # Get encoder output
      encoder_output, hidden, cell = model.encoder(features, feature_lengths)
...
def main(config):
  ...
  best_model_state_dict = torch.load("best_model_no_att_F1.pth", map_location=device) #best_model_att128_b16_29_onf1 #best_model_att128_b16_29_onf1
  model.load_state_dict(best_model_state_dict) #for f1
  # model.load_state_dict(best_model_state_dict['model_state_dict']) # for loss 

  weights_test = torch.zeros(num_classes, dtype=torch.float32)   
  weights_test[0] = class_weights_test[0]
  weights_test[1] = class_weights_test[1]
  weights_test[2] = class_weights_test[2]

  weights_test = weights_test.to(device)
  loss_fn_val = nn.NLLLoss(reduction='none', weight = weights_test)
  ...
  with open("SGlWxxp-IBA_no_att.srt", "w") as f:
    f.write(srt_content)

if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='ASL inference on test data')
